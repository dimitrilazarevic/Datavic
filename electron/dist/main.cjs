...
var BOTTLE_ANALYSIS_KEYS = BOTTLE_ANALYSIS_TEST_TYPE.flatMap((t) => BOTTLE_ANALYSIS_THICKNESS_TYPE.map((b) => `${t}_${b}`));
...
var MATERIAL_ANALYSIS_KEYS = MATERIAL_ANALYSIS_TEST_TYPE.flatMap((t) => MATERIAL_ANALYSIS_TEST_DIRECTION.map((d) => `${t}_${d}`));
...
function registerAppHandlers() {
	...
	electron.ipcMain.handle("get-entity-image", (_event, entity, folderName, ext) => {
		const filePath = path.default.join(path.default.dirname(getDbPath()), entity, folderName, `image.${ext}`);
...
function registerConfigHandlers() {
	...
	electron.ipcMain.handle("config:backup-db", async () => {
		...
		const tmpDbPath = path.default.join(electron.app.getPath("temp"), `datavic-backup-${formatDateForFilename()}.db`);
	...
	electron.ipcMain.handle("config:restore-db", async () => {
		...
		if (!entries.some((e) => e.entryName.endsWith(".db"))) throw new Error("L'archive ne contient aucun fichier .db");
		const currentDbPath = getDbPath();
		const dbDir = path.default.dirname(currentDbPath);
		const backupZipPath = path.default.join(dbDir, `datavic-pre-restore-${formatDateForFilename()}.zip`);
		const tmpDbPath = path.default.join(electron.app.getPath("temp"), `datavic-pre-restore-${formatDateForFilename()}.db`);
		await getClient().backup(tmpDbPath);
		try {
			await new Promise((resolve, reject) => {
...
var tables = {
	bottleType: createSimpleQueries(bottleType, bottleType.bottleTypeId, "bottleType", () => {
		return new Set(getDb().select({ id: bottle.bottleTypeId }).from(bottle).all().map((r) => r.id));
	}),
	brand: createSimpleQueries(brand, brand.brandId, "brand", () => {
		return new Set(getDb().select({ id: bottle.brandId }).from(bottle).all().map((r) => r.id));
	}),
	overbrand: createSimpleQueries(overbrand, overbrand.overBrandId, "overbrand", () => {
		return new Set(getDb().select({ id: bottle.overbrandId }).from(bottle).all().map((r) => r.id));
	}),
	zone: createSimpleQueries(zone, zone.zoneId, "zone", () => {
		return new Set(getDb().select({ id: bottle.zoneId }).from(bottle).all().map((r) => r.id));
	}),
	materialFamily: createSimpleQueries(materialFamily, materialFamily.materialFamilyId, "materialFamily", () => {
		return new Set(getDb().select({ id: material.materialFamilyId }).from(material).all().map((r) => r.id));
	}),
...
function parseBottleAnalysisKey(key) {
	for (const thickness of BOTTLE_ANALYSIS_THICKNESS_TYPE) if (key.endsWith(`_${thickness}`)) return [key.slice(0, -(thickness.length + 1)), thickness];
...
function parseMaterialAnalysisKey(key) {
	for (const direction of MATERIAL_ANALYSIS_TEST_DIRECTION) if (key.endsWith(`_${direction}`)) return [key.slice(0, -(direction.length + 1)), direction];
...
function getByIdWithJoins$1(id) {
	...
	}).from(bottle).innerJoin(bottleType, (0, drizzle_orm.eq)(bottle.bottleTypeId, bottleType.bottleTypeId)).innerJoin(brand, (0, drizzle_orm.eq)(bottle.brandId, brand.brandId)).innerJoin(material, (0, drizzle_orm.eq)(bottle.materialId, material.materialId)).innerJoin(materialFamily, (0, drizzle_orm.eq)(material.materialFamilyId, materialFamily.materialFamilyId)).innerJoin(overbrand, (0, drizzle_orm.eq)(bottle.overbrandId, overbrand.overBrandId)).innerJoin(zone, (0, drizzle_orm.eq)(bottle.zoneId, zone.zoneId)).where((0, drizzle_orm.eq)(bottle.bottleId, id)).get();
	if (!row) return void 0;
	const analyses = getDb().select().from(bottleAnalysis).where((0, drizzle_orm.eq)(bottleAnalysis.bottleId, id)).all();
	const massLossNum = computeMassLossNum(row.bottleTypeConstant, row.bottle.surfaceCm2, row.bottle.thicknessMm);
	return {
		...row.bottle,
...
function updateFolderName$1(id) {
	...
	getDb().update(bottle).set({ folderName }).where((0, drizzle_orm.eq)(bottle.bottleId, id)).run();
	return folderName;
}
var bottleQueries = {
	getAll() {
		try {
			...
			}).from(bottle).innerJoin(bottleType, (0, drizzle_orm.eq)(bottle.bottleTypeId, bottleType.bottleTypeId)).innerJoin(brand, (0, drizzle_orm.eq)(bottle.brandId, brand.brandId)).innerJoin(material, (0, drizzle_orm.eq)(bottle.materialId, material.materialId)).innerJoin(materialFamily, (0, drizzle_orm.eq)(material.materialFamilyId, materialFamily.materialFamilyId)).innerJoin(overbrand, (0, drizzle_orm.eq)(bottle.overbrandId, overbrand.overBrandId)).innerJoin(zone, (0, drizzle_orm.eq)(bottle.zoneId, zone.zoneId)).all().map((row) => {
				const { bottle: b, bottleTypeName, bottleTypeConstant, brandName, overbrandName, zoneName, materialFolderName, materialFamilyName, ...dataFlags } = row;
				const availableData = {};
	...
	create(data) {
		try {
			...
			const row = getDb().transaction((tx) => {
				const inserted = tx.insert(bottle).values({
					...dbData,
					createdAt: now,
					lastModified: now
				}).returning().get();
				if (analysisFiles?.length) for (const af of analysisFiles) {
					const [testType, thicknessType] = parseBottleAnalysisKey(af.analysisKey);
					tx.insert(bottleAnalysis).values({
			...
			if (folderName) {
				...
				if (rawImageContent && row.imageExtension) saveImage("bottles", folderName, row.imageExtension, rawImageContent);
				if (analysisFiles?.length) for (const af of analysisFiles) saveAnalysisFile("bottles", folderName, af.fileName, af.fileContentText);
			}
	...
	update(id, data) {
		try {
			const { rawImageContent, analysisFiles, ...dbData } = data;
			const oldImageExtension = rawImageContent ? getDb().select({ imageExtension: bottle.imageExtension }).from(bottle).where((0, drizzle_orm.eq)(bottle.bottleId, id)).get()?.imageExtension ?? void 0 : void 0;
			getDb().transaction((tx) => {
				tx.update(bottle).set({
					...dbData,
					lastModified: (/* @__PURE__ */ new Date()).toISOString()
				}).where((0, drizzle_orm.eq)(bottle.bottleId, id)).run();
				if (analysisFiles?.length) for (const af of analysisFiles) {
					const [testType, thicknessType] = parseBottleAnalysisKey(af.analysisKey);
					tx.delete(bottleAnalysis).where((0, drizzle_orm.and)((0, drizzle_orm.eq)(bottleAnalysis.bottleId, id), (0, drizzle_orm.eq)(bottleAnalysis.testType, testType), (0, drizzle_orm.eq)(bottleAnalysis.thicknessType, thicknessType))).run();
					tx.insert(bottleAnalysis).values({
						bottleId: id,
						testType,
						thicknessType,
			...
			if (folderName) {
				if (rawImageContent) {
					const row = getDb().select({ imageExtension: bottle.imageExtension }).from(bottle).where((0, drizzle_orm.eq)(bottle.bottleId, id)).get();
					if (row?.imageExtension) {
						if (oldImageExtension && oldImageExtension !== row.imageExtension) deleteImage("bottles", folderName, oldImageExtension);
	...
	delete(id) {
		try {
			const row = getDb().select({ folderName: bottle.folderName }).from(bottle).where((0, drizzle_orm.eq)(bottle.bottleId, id)).get();
			const result = getDb().delete(bottle).where((0, drizzle_orm.eq)(bottle.bottleId, id)).returning().get();
	...
	deleteMany(ids) {
		try {
			const rows = getDb().select({ folderName: bottle.folderName }).from(bottle).where((0, drizzle_orm.inArray)(bottle.bottleId, ids)).all();
			getDb().delete(bottle).where((0, drizzle_orm.inArray)(bottle.bottleId, ids)).run();
	...
	async exportZip(ids) {
		try {
			return exportEntitiesZip("bottles", getDb().select({ folderName: bottle.folderName }).from(bottle).where((0, drizzle_orm.inArray)(bottle.bottleId, ids)).all().map((r) => r.folderName).filter((f) => !!f));
		} catch (err) {
	...
	uploadAnalysis(id, analysisFile) {
		try {
			...
			const row = getDb().select({ folderName: bottle.folderName }).from(bottle).where((0, drizzle_orm.eq)(bottle.bottleId, id)).get();
			if (row?.folderName) saveAnalysisFile("bottles", row.folderName, analysisFile.fileName, analysisFile.fileContentText);
		} catch (err) {
	...
	deleteAnalysis(id, analysisKey) {
		try {
			...
			getDb().delete(bottleAnalysis).where((0, drizzle_orm.and)((0, drizzle_orm.eq)(bottleAnalysis.bottleId, id), (0, drizzle_orm.eq)(bottleAnalysis.testType, testType), (0, drizzle_orm.eq)(bottleAnalysis.thicknessType, thicknessType))).run();
		} catch (err) {
...
function registerBottleHandlers() {
	...
	electron.ipcMain.handle("db:bottle:uploadAnalysis", (_e, id, data) => bottleQueries.uploadAnalysis(id, data));
	electron.ipcMain.handle("db:bottle:deleteAnalysis", (_e, id, key) => bottleQueries.deleteAnalysis(id, key));
}
//#endregion
...
function getByIdWithJoins(id) {
	...
	}).from(material).innerJoin(materialFamily, (0, drizzle_orm.eq)(material.materialFamilyId, materialFamily.materialFamilyId)).innerJoin(supplier, (0, drizzle_orm.eq)(material.supplierId1, supplier.supplierId)).leftJoin(supplier2, (0, drizzle_orm.eq)(material.supplierId2, supplier2.supplierId)).where((0, drizzle_orm.eq)(material.materialId, id)).get();
	if (!row) return void 0;
	const analyses = getDb().select().from(materialAnalysis).where((0, drizzle_orm.eq)(materialAnalysis.materialId, id)).all();
	return {
		...row.material,
		materialFamilyName: row.materialFamilyName,
...
function updateFolderName(id) {
	...
	getDb().update(material).set({ folderName }).where((0, drizzle_orm.eq)(material.materialId, id)).run();
	return folderName;
}
var materialQueries = {
	getAll() {
		try {
			...
			return getDb().select({
				...
				...availableDataColumns
			}).from(material).innerJoin(materialFamily, (0, drizzle_orm.eq)(material.materialFamilyId, materialFamily.materialFamilyId)).innerJoin(supplier, (0, drizzle_orm.eq)(material.supplierId1, supplier.supplierId)).leftJoin(supplier2, (0, drizzle_orm.eq)(material.supplierId2, supplier2.supplierId)).all().map((row) => {
				const { material: m, materialFamilyName, supplierName1, supplierName2, isLinked, ...dataFlags } = row;
				const availableData = {};
	...
	create(data) {
		try {
			...
			const now = (/* @__PURE__ */ new Date()).toISOString();
			const row = getDb().transaction((tx) => {
				const inserted = tx.insert(material).values({
					...dbData,
					createdAt: now,
					lastModified: now
				}).returning().get();
				if (analysisFiles?.length) for (const af of analysisFiles) {
					...
					tx.insert(materialAnalysis).values({
			...
			if (folderName) {
				...
				if (rawImageContent && row.imageExtension) saveImage("materials", folderName, row.imageExtension, rawImageContent);
				if (analysisFiles?.length) for (const af of analysisFiles) saveAnalysisFile("materials", folderName, af.fileName, af.fileContentText);
			}
	...
	update(id, data) {
		try {
			const { rawImageContent, analysisFiles, ...dbData } = data;
			const oldImageExtension = rawImageContent ? getDb().select({ imageExtension: material.imageExtension }).from(material).where((0, drizzle_orm.eq)(material.materialId, id)).get()?.imageExtension ?? void 0 : void 0;
			getDb().transaction((tx) => {
				tx.update(material).set({
					...dbData,
					lastModified: (/* @__PURE__ */ new Date()).toISOString()
				}).where((0, drizzle_orm.eq)(material.materialId, id)).run();
				if (analysisFiles?.length) for (const af of analysisFiles) {
					const [testType, testDirection] = parseMaterialAnalysisKey(af.analysisKey);
					tx.delete(materialAnalysis).where((0, drizzle_orm.and)((0, drizzle_orm.eq)(materialAnalysis.materialId, id), (0, drizzle_orm.eq)(materialAnalysis.testType, testType), (0, drizzle_orm.eq)(materialAnalysis.testDirection, testDirection))).run();
					tx.insert(materialAnalysis).values({
						materialId: id,
						testType,
						testDirection,
			...
			if (folderName) {
				if (rawImageContent) {
					const row = getDb().select({ imageExtension: material.imageExtension }).from(material).where((0, drizzle_orm.eq)(material.materialId, id)).get();
					if (row?.imageExtension) {
						if (oldImageExtension && oldImageExtension !== row.imageExtension) deleteImage("materials", folderName, oldImageExtension);
	...
	delete(id) {
		try {
			const row = getDb().select({ folderName: material.folderName }).from(material).where((0, drizzle_orm.eq)(material.materialId, id)).get();
			const result = getDb().delete(material).where((0, drizzle_orm.eq)(material.materialId, id)).returning().get();
	...
	deleteMany(ids) {
		try {
			const rows = getDb().select({ folderName: material.folderName }).from(material).where((0, drizzle_orm.inArray)(material.materialId, ids)).all();
			getDb().delete(material).where((0, drizzle_orm.inArray)(material.materialId, ids)).run();
	...
	async exportZip(ids) {
		try {
			return exportEntitiesZip("materials", getDb().select({ folderName: material.folderName }).from(material).where((0, drizzle_orm.inArray)(material.materialId, ids)).all().map((r) => r.folderName).filter((f) => !!f));
		} catch (err) {
	...
	uploadAnalysis(id, analysisFile) {
		try {
			...
			const row = getDb().select({ folderName: material.folderName }).from(material).where((0, drizzle_orm.eq)(material.materialId, id)).get();
			if (row?.folderName) saveAnalysisFile("materials", row.folderName, analysisFile.fileName, analysisFile.fileContentText);
		} catch (err) {
	...
	deleteAnalysis(id, analysisKey) {
		try {
			...
			getDb().delete(materialAnalysis).where((0, drizzle_orm.and)((0, drizzle_orm.eq)(materialAnalysis.materialId, id), (0, drizzle_orm.eq)(materialAnalysis.testType, testType), (0, drizzle_orm.eq)(materialAnalysis.testDirection, testDirection))).run();
		} catch (err) {
...
function registerMaterialHandlers() {
	...
	electron.ipcMain.handle("db:material:uploadAnalysis", (_e, id, data) => materialQueries.uploadAnalysis(id, data));
	electron.ipcMain.handle("db:material:deleteAnalysis", (_e, id, key) => materialQueries.deleteAnalysis(id, key));
}
//#endregion
